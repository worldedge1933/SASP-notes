...
当相位差 $phi.alt = plus.minus pi/2$，及 $a=d$ 时，通用振荡器退化为正交振荡器 (quadrature oscillator).
...
在数字实现的递推振荡器中，由于数字表示的有限精度，会导致误差积累，因此需要对幅度和频率进行动态调控以保持系统的稳定性和准确性。

=== 1）幅度调控

由于两个状态变量不一定必然呈现 $90^circle.small$ 的相位差，因此无法简单通过平方和来求幅度。

对于一般通用振荡器，其功率为

$
  P = ( x_1^2 - b/c x_2^2 - 2sqrt(- b/c) x_1 x_2 cos(phi.alt)) / (sin^2 (phi.alt))
$
...
然而，锯齿波本身包含了大量高频成分，因此直接采样会导致严重的混叠失真。

=== 1) 有限合成法

可以使用有限合成办法来实现一个近似的锯齿波

由傅里叶级数展开，可以知道锯齿波的傅里叶系数时 $1/k$

$
  s(n) = - sum_(k=1)^K 1 / k sin(2 pi k f_0 n / F_s)
$

K 为小于 Nyquist 频率的最大整数。计算量随着 K 的增加而增加，且低音的带限锯齿波的计算量更大，因为需要的 K 更大。

=== 2) 抛物波微分法 Differentiated Parabolic Wave Method

我们假设有一抛物波，

$
  p(t) = integral_(- T_0/2)^(t) s(tau) dif tau
$
...
从连续视角下看这些操作，可能觉得多次一举，不会对频谱有任何影响。但实际上当我们将其转为数字实现时，这种操作可以有效的减少混叠。

为了更少的减少混叠，还可以继续使用过采样率技术。先使用过采样率得到锯齿波，再通过低通滤波器，再进行抽取得到最终的锯齿波。
